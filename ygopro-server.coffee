...
class Room
  constructor: (name, @hostinfo) ->
    ...
    if settings.modules.tournament_mode.enabled or @arena or settings.modules.test_mode.watch_public_hand # 0x4: Save chat in cloud replay
...
ygopro.ctos_follow 'CHAT', true, (buffer, info, client, server, datas)->
  ...
  if (msg.length>100)
    ...
    cancel = true
  if not cancel and settings.modules.chatgpt.enabled and room.windbot and not client.is_post_watcher and client.pos < 2 and not client.is_local and room.hostinfo.mode != 2
