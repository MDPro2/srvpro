...
(function() {
  ...
  Room = class Room {
    constructor(name, hostinfo) {
      ...
      if (settings.modules.tournament_mode.enabled || this.arena || settings.modules.test_mode.watch_public_hand) { // 0x4: Save chat in cloud replay
    ...
    spawn(firstSeed) {
      var duel_rule_flags, e, firstSeedBuf, i, j, param, ref;
  ...
  ygopro.stoc_follow('GAME_MSG', true, async function(buffer, info, client, server, datas) {
    var act_pos, card, chain, check, count, cpos, deck_found, dialogText, found, hint_type, id, j, l, len, len1, len2, len3, len4, limbo_found, loc, m, msg_inst, msg_name, n, new_buf, o, oppo_pos, phase, player, playertype, pos, ppos, r_player, reason, record_last_game_msg, ref, ref1, ref2, ref3, ref4, response, room, trigger_location, val, victoryWordPlayerList, win_pos;
    room = ROOM_all[client.rid];
    if (!(room && !client.reconnecting)) {
      return;
    }
    ...
    if (settings.modules.heartbeat_detection.enabled && client.pos === 0) {
      if (msg_inst instanceof YGOProMsg.YGOProMsgChaining) {
        card = msg_inst.code;
        found = false;
        for (n = 0, len3 = long_resolve_cards.length; n < len3; n++) {
  ...
  if (true) {
    ...
    httpRequestListener = async function(request, response) {
      var allowHeaders, archiveStream, base, buffer, death_room_found, duellog, e, err, error, filename, getpath, j, l, len, len1, len2, len3, m, n, parseQueryString, pass_validated, player, requestHeaders, ret_keys, room, roomsjson, success, u, urlObj;
